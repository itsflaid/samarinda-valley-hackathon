...
import { dummyRegions, statusColors, type RegionStatus } from "@/lib/mock-data";
...
export default function WilayahPage() {
  ...
  const total = dummyRegions.length;
  const countAman = dummyRegions.filter((r) => r.status === "AMAN").length;
  const countWaspada = dummyRegions.filter((r) => r.status === "WASPADA").length;
  const countSiaga = dummyRegions.filter((r) => r.status === "SIAGA").length;

  const filtered = useMemo(() => {
    return dummyRegions.filter((region) => {
      const matchesSearch =
        !search ||
                ...
                <div className="mt-4 space-y-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-sm">
                      {region.symptomReports !== null
                        ? `${region.symptomReports} laporan`
                        : "Data belum tersedia"}
                    </p>
                  </div>
